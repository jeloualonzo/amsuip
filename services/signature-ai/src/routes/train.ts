...
import { TrainingRequest, TrainingResponse } from '../types';
...
export async function trainStudent(
  request: FastifyRequest<{ Params: TrainParams }>,
  reply: FastifyReply
): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      if (imageUrls.length < CONFIG.MIN_SAMPLES_FOR_TRAINING) {
        reply.code(400).send({
          success: false,
          error: 'Insufficient training data',
          message: `At least ${CONFIG.MIN_SAMPLES_FOR_TRAINING} signature images are required for training. Found ${imageUrls.length}.`,
        });
        return;
      }
      ...
      for (let i = 0; i < imageUrls.length; i++) {
        ...
        try {
          ...
          if (embeddings.length < CONFIG.MIN_SAMPLES_FOR_TRAINING * 2) {
            try {
      ...
      if (embeddings.length > 1) {
        ...
        threshold = Math.min(CONFIG.DEFAULT_THRESHOLD, mean + 1.5 * stdDev);
