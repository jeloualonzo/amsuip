...
export const fetchStudents = async (program?: string, year?: string, section?: string): Promise<Student[]> => {
  try {
    ...
    // Get all matching students based on program and year first
    let query = supabase
      .from('students')
      .select('*')
    ...
    if (program && program !== 'All Programs') {
      query = query.eq('program', program.trim());
    }

    // Apply year level filter if it's a specific year (not 'all')
    if (year && year !== 'All Years') {
      // Extract just the number from '1st Year', '2nd Year', etc.
      const yearNum = year.match(/\d+/)?.[0];
      if (yearNum) {
        query = query.eq('year_level', parseInt(yearNum, 10));
      }
    }

    // Execute the query to get all matching students first
...
export const fetchSessionStudents = async (sessionId: number): Promise<SessionWithStudents> => {
  ...
  const isAllValue = (value?: string) => 
    !value || value.toLowerCase().includes('all') || value === '';
  ...
  // First, count total matching students without pagination
  const { count, error: countError } = await supabase
    .from('students')
    .select('*', { count: 'exact', head: true })
    .match({
      ...(session.program && !isAllValue(session.program) && { program: session.program.trim() }),
      ...(session.year && !isAllValue(session.year) && { year: session.year.trim() }),
      ...(sectionValue && !isAllValue(sectionValue) && { section: sectionValue.trim() })
    });
  ...
  // Add debug query to see sample student data
  const { data: debugSample } = await supabase
    .from('students')
    .select('*')
    .limit(5);
  console.log('Sample student data:', debugSample);

  // Count already retrieved in the first query

  // Helper function to try alternative section name formats
  const tryAlternativeFormats = async (sectionFilter: string, program: string, year: string) => {
    console.log('Trying alternative section formats...');
    
    // Try with space after program name (e.g., "BSBIO 3C" -> "BSBIO  3C")
    const altFormat1 = sectionFilter.replace(' ', '  ');
    console.log('Trying format with double space:', altFormat1);
    const { data: altData1 } = await supabase
      .from('students')
      .select('*')
      .eq('program', program.trim())
      .eq('year', year.trim())
      .eq('section', altFormat1);
      
    if (altData1?.length) return altData1;
    
    // Try with different case (e.g., "bsbio 3c")
    const altFormat2 = sectionFilter.toLowerCase();
    console.log('Trying lowercase format:', altFormat2);
    const { data: altData2 } = await supabase
      .from('students')
      .select('*')
      .eq('program', program.trim())
      .eq('year', year.trim())
      .ilike('section', altFormat2);
      
    if (altData2?.length) return altData2;
    
    // Try with just the section number/letter
    const sectionNumber = sectionFilter.match(/\d+[A-Za-z]*/)?.[0];
    if (sectionNumber) {
      console.log('Trying with just section number/letter:', sectionNumber);
      const { data: altData3 } = await supabase
        .from('students')
        .select('*')
        .eq('program', program.trim())
        .eq('year', year.trim())
        .ilike('section', `%${sectionNumber}%`);
        
      if (altData3?.length) return altData3;
    }
  ...
  // First, try to get all students with the given filters
  try {
    ...
    if (sectionValue && !isAllValue(sectionValue)) {
      ...
      // Extract section number (e.g., '1C' from 'BSIS 1C')
      const sectionNumber = sectionFilter.split(' ').pop() || '';
      
      if (sectionNumber && sectionNumber !== sectionFilter) {
        // Try both full section and just the number with OR condition
        query = query.or(`section.ilike.%${sectionFilter}%,section.ilike.%${sectionNumber}%`);
      } else {
    ...
    if (allStudents.length === 0 && sectionValue && !isAllValue(sectionValue)) {
      ...
      for (const format of uniqueFormats) {
        ...
        const { data: exactMatch } = await altQuery.eq('section', format);
        ...
        const { data: partialMatch } = await altQuery.ilike('section', `%${format}%`);
